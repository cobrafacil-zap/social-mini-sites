...
export async function updateSite(id: string, patch: Partial<Site>): Promise<{ error?: string }> {
  ...
  if ((patch as Record<string, unknown>).slugTouched !== undefined) {
    writable.slugTouched = (patch as Record<string, unknown>).slugTouched;
  }
  if (patch.status !== undefined) writable.status = patch.status;
  if (patch.template !== undefined) writable.template = patch.template;
  if (patch.company !== undefined) writable.company = patch.company;
  if (patch.location !== undefined) writable.location = patch.location;
  if (patch.hours !== undefined) writable.hours = patch.hours;
  if (patch.gallery !== undefined) writable.gallery = patch.gallery;
  if (patch.buttons !== undefined) writable.buttons = patch.buttons;
  if (patch.customization !== undefined) writable.customization = patch.customization;
